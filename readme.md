...
- **优雅资源管理**：提供 `Unregister` 方法清理资源。
- **唯一 ID 生成**：基于 UUID 生成带类型前缀的唯一 ID。
- **上下文支持**：提供 `ConsumeContext` 方法支持上下文取消。
- **完整文档**：所有导出类型和方法都有完整的 godoc 注释。
...
func main() {
    ...
    // 或使用非阻塞方式
    if msg, ok, err := subscriber.TryConsume(); ok && err == nil {
        fmt.Printf("Received: %s\n", string(msg.Data))
    } else if err != nil {
        fmt.Printf("Error trying to consume: %v\n", err)
    }

    // 6. 清理资源
    subscriber.Unregister() // 关闭通道并注销订阅者
}
```
...
| `Consume() (Message, error)` | 阻塞式消费一条消息，返回消息和错误。 |
| `ConsumeContext(ctx context.Context) (Message, error)` | 带上下文的阻塞消费，支持超时或取消。 |
| `TryConsume() (Message, bool, error)` | 非阻塞消费，返回 `(消息, 是否收到, 错误)`。 |
| `Unregister()` | 关闭消息通道并注销订阅者。 |
...
| `ID` | `PubSubID` | 消息唯一标识符 |
| `Data` | `[]byte` | 消息内容 |
| `Topics` | `[]string` | 消息所属主题列表 |
| `Publisher` | `PubSubID` | 发布者ID |
...
2. **资源清理**：
   - 建议在 `defer` 中调用 `subscriber.Unregister()` 或 `publisher.Unregister()`。
   - `Unregister()` 会关闭 `MsgChan`，后续 `Receive` 调用将 panic。
...
4. **错误处理**：
   - `Consume` 和 `TryConsume` 方法现在返回错误，需要适当处理。
   - 当订阅者关闭时，`Consume` 和 `TryConsume` 会返回 `ErrSubscriberClosed`。

5. **ID 唯一性**：基于 `github.com/google/uuid` 保证全局唯一。

---

## 📚 文档

完整的 API 文档可以通过 `go doc` 查看：
...
或者 [在线查看](https://pkg.go.dev/github.com/Jinvic/pubsub)。
